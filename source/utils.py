...
def read_web(url, user_agent_list):
...
def recursive_crawl(url, user_agent_list, atc_dict):
    time.sleep(0.001)
    ...
    for site in urls:
        if site['href'].find('&') < 0:
            pass
        elif site.text == 'here' or site.text == 'Guidelines':
            return False
        else:
            code = site['href'][8:site['href'].find('&')]
            atc_dict[code] = site.text
            if len(code) == 7:
                pass
            else:
